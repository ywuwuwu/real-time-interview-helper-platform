...
async def test_simplified_resume_parsing():
    """测试简化的简历解析功能"""
    print("🚀 测试简化的简历解析功能...")
...
- Contributions: Spark plugins, Feature Store reference implementation"""
    ...
    # 保存测试简历文件
    test_resume_path = "test_resume_simplified_parsing.txt"
    ...
    try:
        ...
        if 'skill_levels' in resume_data:
            print(f"\n🎯 技能等级:")
            for skill, level in resume_data['skill_levels'].items():
                print(f"  - {skill}: {level}")
        
        if 'detailed_analysis' in resume_data:
            print(f"\n📝 详细分析:")
            print(f"  {resume_data['detailed_analysis']}")
        ...
        with open('simplified_resume_parsing_test_result.json', 'w', encoding='utf-8') as f:
            json.dump(test_result, f, ensure_ascii=False, indent=2)
        
        print(f"\n💾 测试结果已保存到 simplified_resume_parsing_test_result.json")
        print("\n🎉 简化的简历解析测试成功！")
...
if __name__ == "__main__":
    ...
    asyncio.run(test_simplified_resume_parsing())
