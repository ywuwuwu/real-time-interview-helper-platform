...
async def test_data_sync():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"  - 经验年数: {result.get('experience_years')}")
            print(f"  - 技能列表: {result.get('skills')}")
            ...
            if get_response.status_code == 200:
                ...
                # 3. 模拟简历上传后的数据更新
                print("\n📄 模拟简历上传后的数据更新...")
                updated_skills = ["Python", "TensorFlow", "PyTorch", "scikit-learn", "Apache Spark", "Kafka", "Hadoop", "Docker", "Kubernetes", "AWS"]
                updated_experience = 4
                ...
                with open('data_sync_test_result.json', 'w', encoding='utf-8') as f:
                    json.dump(test_result, f, ensure_ascii=False, indent=2)
                
                print(f"\n💾 测试结果已保存到 data_sync_test_result.json")
                print("\n🎉 数据同步测试成功！")
